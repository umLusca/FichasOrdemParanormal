    ...
    <div class="card h-100 w-100 bg-black border-light">
        <div class="card-body p-0">
            <div class="card-header border-0">
                <div class="card-title fs-3 text-center font6">Anotações <span id="syncnotes" class="invisible"><i class="fas"></i></span></div>
            </div>
					...
					foreach ($nt as $r):?>
                        <li class="nav-item title" role="presentation">
                            <button class="nav-link" id="a<?= $r["id"] ?>-tab" data-bs-toggle="pill"
                                    data-bs-target="#a<?= $r["id"] ?>" type="button" role="tab"><?= $r["nome"] ?></button>
                        </li>
					<?php endforeach; ?>
					<?php if ($nt->num_rows < 5) { ?>
                        <li class="nav-item" role="presentation">
                            <button type="button" class="btn text-success" onclick="addnote();"><i class="fa-regular fa-square-plus"></i> Adicionar
                            </button>
                        </li>
					<?php } ?>
                </ul>
                <div class="tab-content" id="notes">
					<?php foreach ($nt as $r): ?>
                        <div class="tab-pane fade show" id="a<?= $r["id"] ?>" role="tabpanel"
                            ...
                            <textarea name="nota[]" class="note form-control bg-black text-light" rows="10" cols="80" maxlength="5000"><?= $r["notas"] ?></textarea>
